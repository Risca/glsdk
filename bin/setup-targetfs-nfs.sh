...
extract_fs() {
    fstar=`ls -1 $cwd/../filesystem/arago*sdk*console*omap5*.tar.gz`
    me=`whoami`
    sudo mkdir -p $1
    check_status
...
echo "setenv bootargs 'root=/dev/nfs nfsroot=$ipaddr:$dst console=ttyO2,115200n8 earlyprintk rw ip=dhcp'" >> boot.script.nfs
